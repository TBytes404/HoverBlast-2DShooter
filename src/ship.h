...
typedef struct {
  ...
  int score, health, fireRate, lastShootFrame, lastBlastFrame, currentBlastRect,
      totalBlastRects;
} Ship;
...
void drawShip(Ship *ship) {
  (DrawTextureEx(shipTexture,
                 ship->isEnemy ? (Vector2){ship->rect.x + ship->rect.width,
                                           ship->rect.y + ship->rect.height}
                               : (Vector2){ship->rect.x, ship->rect.y},
                 ship->isEnemy ? 180 : 0, ship->rect.width / shipTexture.width,
                 WHITE));
}
...
bool overBlast(Ship *ship) {
  ...
  if (ship->lastBlastFrame < (frameRate / ship->totalBlastRects))
    ...
    (ship->lastBlastFrame = 0), (ship->currentBlastRect < ship->totalBlastRects)
...
static void moveShip(Ship *ship) {
  ...
  if (ship->rect.x < 0)
    ...
    ship->xSpeed > 0 ? (ship->xSpeed *= -1)
                     : (ship->rect.x = arenaWidth() - ship->rect.width);

  // vertical collision
  if (ship->rect.y < (ship->isEnemy ? 0 : arenaHeight()))
    ship->ySpeed < 0 ? (ship->ySpeed *= -1)
                     : (ship->rect.y = (ship->isEnemy ? 0 : arenaHeight()));
  else if (ship->rect.y > (ship->isEnemy ? arenaHeight() : GetScreenHeight()) -
                              ship->rect.height)
    ship->ySpeed > 0
        ? (ship->ySpeed *= -1)
        : (ship->rect.y = (ship->isEnemy ? arenaHeight() : GetScreenHeight()) -
                          ship->rect.height);
}

void handleMovement(Ship *ship) {
  if (!ship->isEnemy)
    accelerate(&ship->xSpeed, IsKeyDown(KEY_LEFT) || IsKeyDown(KEY_A),
               IsKeyDown(KEY_RIGHT) || IsKeyDown(KEY_D), ship->accel),
        accelerate(&ship->ySpeed, IsKeyDown(KEY_UP) || IsKeyDown(KEY_W),
                   IsKeyDown(KEY_DOWN) || IsKeyDown(KEY_S), ship->accel);
  else if (ship->direc != NONE)
    accelerate(&ship->xSpeed, ship->direc == LEFT, ship->direc == RIGHT,
               ship->accel),
        accelerate(&ship->ySpeed, ship->direc == UP, ship->direc == DOWN,
                   ship->accel);
  moveShip(ship);
}
  ...
  if (ship->lastShootFrame < frameRate / ship->fireRate)
    ...
    for (int i = 0; i < maxShots; i++)
      if (!ship->shots[i].active) {
        ...
        ship->shots[i].pos.y =
            ship->rect.y + (ship->isEnemy ? ship->rect.height : 0);
